...
class Wiip_Base32 {
	 ...
	 */
	const CHARSET_RFC3548 = 'ABCDEFGHIJKLMNOPQRSTUVWXZY234567';
	 ...
	 *
	 */
	const CHARSET_SAFE = '0123456789ABCDEFGHJKMNPQRSTVWXYZ';
	 ...
	 *
	 */
	const CHARSET_09AV = '0123456789ABCDEFGHIJKLMNOPQRSTUV';

    /**
     * On a enlevé le 0, O, 1 et I pour éviter les confusions.
     */
    const CHARSET_WIIP = '4DQS9FBGZ8L6MNT5CKXP7RYA2HJEV3UW';
	 ...
	 * @see setCharset
	 */
	public function __construct($charset = self::CHARSET_RFC3548) {
	...
	public function fromString($data) {
		$inByteSize = 8;
		$outByteSize = 5;
		$alphabetChars = str_split($this->_charset);
		...
		while ($i < strlen($data)) {
			// Use ord() to get byte value
			$current_byte = ord($data[$i]);

			// Check if the current 5-bit chunk will span a byte boundary
			if ($index > ($inByteSize - $outByteSize)) {
				if (($i + 1) < strlen($data)) {
					$next_byte = ord($data[$i + 1]);
				} else {
					$next_byte = 0;
				}
	...
	public function toString($str) {
		...
		if ($this->_charset == self::CHARSET_SAFE) {
	 ...
	 * @throws Exception
	 */
	public function setCharset($charset = self::CHARSET_RFC3548) {
